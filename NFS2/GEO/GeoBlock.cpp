    ...
    template <> bool GeoBlock<PS1>::_SerializeIn(std::ifstream &ifstream) {
        ...
        while (true) {
            ...
            for (uint32_t vert_Idx = 0; vert_Idx < geoBlockHeader->nVerts; ++vert_Idx) {
                verts.emplace_back(rotationMatrix * glm::vec3(vertices[vert_Idx].x / carScaleFactor, vertices[vert_Idx].y / carScaleFactor,
            ...
            glm::vec3 center =
                glm::vec3((geoBlockHeader->position[0] / 256.0f) / carScaleFactor, (geoBlockHeader->position[1] / 256.0f) / carScaleFactor,
                          (geoBlockHeader->position[2] / 256.0f) / carScaleFactor);
            car_meshes.emplace_back(CarModel(PS1_PART_NAMES[part_Idx], verts, uvs, texture_indices, texMapStuff, norms, indices, center,
